...
`configure_optimizers()` creates a [Muon, AdamW] optimizer pair using TEON + Dion: cross-layer Q/K/V stacking (K=2) uses TEON with Polar Express orthogonalization, out_proj/MLP weights use Dion low-rank updates with error feedback, and AdamW handles embeddings, normalization params, biases, and remaining params. Attention uses separate `q_proj`, `k_proj`, `v_proj` projections. Config fields: `muon_lr` (default 0.02), `muon_momentum` (0.95), `ns_steps` (5), `dion_rank_fraction` (0.25), `dion_power_iters` (1).
